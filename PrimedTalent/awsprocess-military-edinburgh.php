...
if (isset($_POST['btn-send'])) {
    ...
    $postData = $_POST;
    $subject = "AWS (Military-Edinburgh)";
    $name = $_POST['name'];
    $email = $_POST['email'];
    $Phone = $_POST['phone'];
    // $Exp = $_POST['Exp'];
    ...
    if (!empty($email) && !empty($name) && !empty($subject) && !empty($Msg2)) {
        ...
        if (!empty($_FILES["attachment"]["name"])) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["attachment"]["tmp_name"], $targetFilePath)) {
                    ...
                    $uploadStatus = 0;
                    // header('location:AWS-Military-Edinburgh?error');
                    // $statusMsg = "Sorry, there was an error uploading your file.";
                }
            } else {
                $uploadStatus = 0;
                header('location:AWS-Military-Edinburgh?fileerror');
        ...
        if ($uploadStatus == 1) {
            ...
            // Subject
            $emailSubject = "AWS(Military-Edinburgh)";
            ...
            if ($mail) {
                $postData = '';
                header("location:AWS-Military-Edinburgh?success");
            } else {
                header('location:AWS-Military-Edinburgh?error');
            }
        }
    ...
    header("location:AWS-Military-Edinburgh");
}
