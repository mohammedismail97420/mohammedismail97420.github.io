...
if (isset($_POST['btn-send'])) {
    ...
    $postData = $_POST;
    $subject = "AWS (Military-Newcastle)";
    $name = $_POST['name'];
    $email = $_POST['email'];
    $Phone = $_POST['phone'];
    // $Exp = $_POST['Exp'];
    ...
    if (!empty($email) && !empty($name) && !empty($subject) && !empty($Msg2)) {
        ...
        if (!empty($_FILES["attachment"]["name"])) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["attachment"]["tmp_name"], $targetFilePath)) {
                    ...
                    $uploadStatus = 0;
                    // header('location:AWS-Military-Newcastle?error');
                    // $statusMsg = "Sorry, there was an error uploading your file.";
                }
            } else {
                $uploadStatus = 0;
                header('location:AWS-Military-Newcastle?fileerror');
        ...
        if ($uploadStatus == 1) {
            ...
            // Subject
            $emailSubject = "AWS(Military-Newcastle)";
            ...
            if ($mail) {
                $postData = '';
                header("location:AWS-Military-Newcastle?success");
            } else {
                header('location:AWS-Military-Newcastle?error');
            }
        }
    ...
    header("location:AWS-Military-Newcastle");
}
