...
if (isset($_POST['btn-send'])) {
    ...
    $postData = $_POST;
    $subject = "AWS (Military-Southampton)";
    $name = $_POST['name'];
    $email = $_POST['email'];
    $Phone = $_POST['phone'];
    // $Exp = $_POST['Exp'];
    ...
    <p><b>Number of years: </b>' . $Noy . '</p>
    <p><b>Number of months: </b>' . $Nom . '</p>
    <p><b>Last job role: </b>' . $Ljr . '</p>
    ...
    if (!empty($email) && !empty($name) && !empty($subject) && !empty($Msg2)) {
        ...
        if (!empty($_FILES["attachment"]["name"])) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["attachment"]["tmp_name"], $targetFilePath)) {
                    ...
                    $uploadStatus = 0;
                    // header('location:AWS-Military-Southampton?error');
                    // $statusMsg = "Sorry, there was an error uploading your file.";
                }
            } else {
                $uploadStatus = 0;
                header('location:AWS-Military-Southampton?fileerror');
        ...
        if ($uploadStatus == 1) {
            ...
            // Subject
            $emailSubject = "AWS(Military-Southampton)";
            ...
            if ($mail) {
                $postData = '';
                header("location:AWS-Military-Southampton?success");
            } else {
                header('location:AWS-Military-Southampton?error');
            }
        }
    ...
    header("location:AWS-Military-Southampton");
}
