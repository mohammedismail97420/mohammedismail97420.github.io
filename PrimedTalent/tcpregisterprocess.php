...
if ($conn->connect_error) {
    ...
    if(isset($_POST['btn-sendtcp']))
    {
	   ...
	   $selectsql = "SELECT coursename,price FROM   `tcp_course_master_uk` where courseid='$courseId'";
	  ...
	  /* $coursesplit = explode(" - ", $Course);
       ...
       if(empty($ParentEmail) || empty($ParentName) || empty($Phone) || empty($KidName) || empty($Grade) || empty($Laptop) || empty($Course) ||empty($Timezone))
	   
       {
           header('location:tcpregister.php?error');
       }
       else
       {
		   ...
		   $sql = "INSERT INTO tcp_user (parentemail, parentname, parentphone, kidname,course,grade,laptop,amount,createddt) VALUES ('$ParentEmail', '$ParentName', '$Phone','$KidName','$courseId','$Grade','$Laptop','$finalPrice','$timestamp')";
			...
			$sql = "INSERT INTO tcp_transaction_details (userid, amount, currency,createddt) VALUES ('$userid', '$finalPrice', 'GBP','$timestamp')";
			if (mysqli_query($conn, $sql)) {
